...
#define SQUARE_WAVE_TABLE_LEN 2048
#define SAMPLES_PER_BUFFER 256

static int16_t square_wave_table[SQUARE_WAVE_TABLE_LEN];
...
int main() {
    ...
    for (int i = 0; i < SQUARE_WAVE_TABLE_LEN; i++) {
        if ((i / 2) > (SQUARE_WAVE_TABLE_LEN >> 2)) {
            square_wave_table[i] = INT16_MAX;
        } else {
            square_wave_table[i] = INT16_MIN;
        }
    }
    ...
    uint32_t pos = 0;
    uint32_t pos_max = 0x10000 * SQUARE_WAVE_TABLE_LEN;
    uint vol = 128;
    while (true) {
        ...
        if (c >= 0) {
            ...
            if (c == ']' && step < (SQUARE_WAVE_TABLE_LEN / 16) * 0x20000) step += 0x10000;
            if (c == 'q') break;
        ...
        for (uint i = 0; i < buffer->max_sample_count; i++) {
            samples[i] = (vol * square_wave_table[pos >> 16u]) >> 8u;
            pos += step;
